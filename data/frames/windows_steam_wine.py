...
class WindowsSteamWineFrame(ttk.Frame):
    def __init__(self, parent, controller, path_manager):
        ...
        self.icons = []
        self.games_data = []  # Store game data
        self.game_widgets = []  # Store the button frames
        self.theme = self.load_theme() # load_theme will now use path_manager
        self.configure_style()

        self.last_width = 0  # Track last known width
        self.initial_load_complete = False  # Flag to track initial load
        self.current_menu = None  # To keep track of the currently open menu
        self._redraw_pending = False # Flag to prevent multiple redraws

        self["style"] = "WindowsSteamGame.TFrame"
        self.bind("<Visibility>", self.on_visibility_change)  # Add visibility handler

        # Bind to the main window's configure event
        self.controller.bind("<Configure>", self.on_main_window_configure)

        self.setup_ui()
        self.load_games() # load_games will now use path_manager
        self.after(200, self.perform_initial_redraw)  # Delay initial redraw
    ...
    def open_edit_game_dialog(self, index):
        ...
        def themed_label(master, text, row, col=0, sticky="w", padx=5, pady=5):
            label = ttk.Label(master, text=text, style="WindowsSteamGame.TLabel")
        ...
        try:
            ...
            ttk.Button(
                dialog,
                text="Browse",
                style="WindowsSteamGame.TButton",
            ...
            def save_changes():
                ...
                if new_image_path:
                    ...
                    IMAGE_DIR = self.path_manager.get_path("data") / "games" / "windowsandsteam" / "assets"
                ...
                SCRIPT_DIR = self.path_manager.get_path("data") / "games" / "windowsandsteam" / "run"
                ...
                DATA_FILE = self.path_manager.get_path("data") / "games" / "windowsandsteam" / "windowsandsteam.json"
                try:
            ...
            ttk.Button(
                dialog, text="Save Changes", style="WindowsSteamGame.TButton", command=save_changes
    ...
    def on_main_window_configure(self, event=None):
        ...
        if current_width > 0 and abs(current_width - self.last_width) > 10:
    ...
    def delayed_redraw(self):
        ...
        self.after(100, self._perform_redraw)
    ...
    def force_redraw(self):
        ...
        # Destroy grid frame if needed
        if hasattr(self, "grid_frame"):
            self.grid_frame.destroy()

        # Recreate grid frame
        self.grid_frame = ttk.Frame(self.canvas, style="WindowsSteamGame.TFrame")
    ...
    def load_theme(self):
        ...
        theme_file_path = self.path_manager.get_path("themes") / "cosmictwilight" / "styles" / "windowsandsteam.json"
    ...
    def configure_style(self):
        ...
        style.configure("WindowsSteamGame.TFrame", background=bg)
        style.configure(
            "WindowsSteamGame.TLabel",
            background=bg,
            foreground=fg,
    ...
    def setup_ui(self):
        ...
        controls_frame = ttk.Frame(self, style="WindowsSteamGame.TFrame")
        ...
        # Create canvas for the game grid (without scrollbar)
        ...
        # Initially create the grid frame
        self.grid_frame = ttk.Frame(self.canvas, style="WindowsSteamGame.TFrame")
    ...
    def on_canvas_resize(self, event):
        """Handle canvas resize and only redraw if width has changed."""
    ...
    def redraw_grid(self):
        if getattr(self, "_is_redrawing", False):
            return  # Already redrawing, skip
        ...
        try:
            ...
            for col in range(columns):
                self.grid_frame.columnconfigure(col, weight=1, uniform="windowssteam_cols")
    ...
    def open_add_game_dialog(self):
        ...
        dialog = tk.Toplevel(self)
        dialog.title("Add Windows/Steam Game")
        dialog.configure(bg=self.theme.get("background", "#1e1e1e"))
        ...
        SCRIPT_DIR_EXAMPLE = self.path_manager.get_path("data") / "games" / "windowsandsteam" / "run"
        ...
        def save():
            ...
            run_command = "#!/bin/bash\n# Add your game launch command here.\n# Example: wine C:\\'Program Files (x86)'\\'Steam'\\'steamapps'\\'common'\\'MyGame'\\'Game.exe'"
        ...
        ttk.Button(dialog, text="Save", style="WindowsSteamGame.TButton", command=save).grid(
    ...
    def sort_games(self, event=None, trigger_redraw=True):
        ...
        # "Date Added" keeps current order
    ...
    def add_game_icon(self, entry):
        try:
            ...
            button_frame = ttk.Frame(self.grid_frame, style="WindowsSteamGame.TFrame")

            # Create image label
            img_label = ttk.Label(button_frame, image=photo, style="WindowsSteamGame.TLabel")
    ...
    def run_game(self, script_path):
        ...
        try:
            ...
            if os.name == 'nt':  # Windows
                ...
                # Use subprocess.Popen without .wait() to not block the GUI
    ...
    def get_recently_played_games(self, num_items=15):
        ...
        # Ensure 'image' and 'exec' are included for dashboard display
        return [
            dict(name=g["name"], last_played=g["last_played"], image=g.get("image"), exec=g["exec"])
